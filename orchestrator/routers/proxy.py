...
SERVICE_URLS = {
    ...
    "dashboard": _resolve_service_url(
        "dashboard-aggregator",
        "DASHBOARD_AGGREGATOR_URL",
        "",
    ),
    "dashboard-aggregator": _resolve_service_url(
        "dashboard-aggregator",
        "DASHBOARD_AGGREGATOR_URL",
        "",
    ),
...
SERVICE_API_KEYS = {
    ...
    "dashboard": os.getenv("DASHBOARD_AGGREGATOR_API_KEY") or os.getenv("DASHBOARD_API_KEY"),
    "dashboard-aggregator": os.getenv("DASHBOARD_AGGREGATOR_API_KEY") or os.getenv("DASHBOARD_API_KEY"),
...
# Dashboard aggregator routes - handle both /dashboard and /dashboard-aggregator
@router.api_route("/dashboard/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"])
async def proxy_dashboard(path: str, request: Request):
    """Proxy requests to dashboard-aggregator service"""
    body = await request.body() if request.method in ["POST", "PUT", "PATCH"] else None
    # Map frontend paths to actual dashboard-aggregator endpoints
    if path == "aggregated-stats":
        path = "api/v1/aggregated-stats"
    elif not path.startswith("api/"):
        path = f"api/v1/{path}"
    return await proxy_request("dashboard", path, request, request.method, body)

@router.api_route("/dashboard-aggregator/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"])
async def proxy_dashboard_aggregator(path: str, request: Request):
    """Proxy requests to dashboard-aggregator service (alternate route)"""
    body = await request.body() if request.method in ["POST", "PUT", "PATCH"] else None
    # MCP routes pass through without rewriting
...
# Database routes - intelligent routing based on operation
@router.api_route("/db/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"])
async def proxy_database(path: str, request: Request):
    """
    Intelligently proxy database requests:
    - READ operations (GET, query) -> dashboard-aggregator
    - WRITE operations (POST, PUT, DELETE) -> db-manager

    Trust the services to handle schema via DBSchema-Manager
    """
    body = await request.body() if request.method in ["POST", "PUT", "PATCH", "DELETE"] else None

    # Route based on operation type - NO MOCK DATA!
    if "query" in path or request.method == "GET":
        # READ operations go to dashboard-aggregator
        # Dashboard-aggregator will consult DBSchema-Manager for schema
        return await proxy_request("dashboard-aggregator", path, request, request.method, body)
    else:
        # WRITE operations go to db-manager
        # DB-Manager will consult DBSchema-Manager for schema
