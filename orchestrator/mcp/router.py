...
from .auth import MCPAuthManager, MCPUser, UserTier
...
def setup_mcp(
    ...
    """
    global _tool_executor, _auth_manager, _rate_limiter, _spine

    _tool_executor = MCPToolExecutor(service_urls, internal_api_key)
    _auth_manager = MCPAuthManager(redis_client=redis_client)
...
async def get_mcp_user(
    ...
    # Route auth through the spine when available.
    if _spine is not None:
        spine_user = await _spine.auth.validate(key, mode="core")
        if not spine_user:
            raise HTTPException(status_code=401, detail="Invalid credentials")

        # Custom implementations may wrap a richer user model in `extra`.
        wrapped = spine_user.extra.get("mcp_user") if spine_user.extra else None
        if wrapped is not None:
            return wrapped

        # Synthesize a compatible user for downstream code that expects
        # tier / credit / daily-limit fields.
        return MCPUser(
    ...
    # Fallback for the brief window before setup_mcp() completes.
    if not _auth_manager:
        raise HTTPException(status_code=503, detail="MCP not initialized")

    if key.startswith("nmcp_") or key.startswith("ncmcp_"):
        user = await _auth_manager.validate_api_key(key)
    elif key.count(".") == 2:
        user = await _auth_manager.validate_jwt(key)
    else:
        user = await _auth_manager.validate_api_key(key)  # unknown shape → let it 401
...
async def get_usage(user: MCPUser = Depends(get_mcp_user)):
    ...
    return {
        ...
        # NovoWorkbench status bar) don't need a separate dashboard-aggregator
...
# =============================================================================
# Admin Endpoints (proxy to dashboard-aggregator)
# =============================================================================

# Dashboard-aggregator internal URL
...
async def create_org(
    ...
    Proxies to dashboard-aggregator.
    """
...
# =============================================================================
# Billing proxy — forward /mcp/billing/* to dashboard-aggregator
# =============================================================================
# Stripe webhook, checkout, subscribe, and portal all live in
# dashboard-aggregator. We proxy them here so the single external
# URL (api.novomcp.com/mcp/billing/*) reaches the handler.

@router.post("/billing/webhook")
async def billing_webhook_proxy(request: Request):
    """Proxy Stripe webhook to dashboard-aggregator.

    Passes raw body + stripe-signature header untouched so
    dashboard-aggregator can verify the Stripe signature.
    """
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{DASHBOARD_AGGREGATOR_URL}/mcp/billing/webhook",
            content=body,
            headers=headers,
            timeout=30.0,
        )
...
async def _proxy_billing_post(request: Request, path: str):
    """Forward an authenticated billing POST to dashboard-aggregator."""
    body = await request.body()
    headers = {"Content-Type": "application/json"}
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{DASHBOARD_AGGREGATOR_URL}/mcp/billing/{path}",
            content=body,
            headers=headers,
            timeout=30.0,
        )
...
async def billing_checkout_proxy(request: Request):
    """Proxy credit-pack checkout (starter/researcher/professional) to dashboard-aggregator."""
    return await _proxy_billing_post(request, "checkout")


@router.post("/billing/subscribe")
async def billing_subscribe_proxy(request: Request):
    """Proxy Team subscription checkout to dashboard-aggregator."""
    return await _proxy_billing_post(request, "subscribe")


@router.post("/billing/portal")
async def billing_portal_proxy(request: Request):
    """Proxy Stripe customer portal session to dashboard-aggregator."""
    return await _proxy_billing_post(request, "portal")
