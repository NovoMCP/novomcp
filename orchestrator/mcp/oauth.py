...
from .auth import validate_via_spine
...
def setup_oauth(redis_client=None):
    """Initialize OAuth with the pluggable spine and optional Redis persistence.

    Auth routes through the spine (LocalAuthGate by default).
    """
    global _redis, _spine
    _redis = redis_client
    _spine = build_spine()
...
async def authorize_post(
    ...
    if not _spine:
        logger.error("[OAuth:authorize:POST] spine not initialized")
        raise HTTPException(status_code=503, detail="OAuth not initialized")

    # Validate the API key through the spine
