...
def setup_mcp_root(tool_executor: MCPToolExecutor):
    """Initialize MCP root handler. Auth routes through the pluggable spine."""
    global _tool_executor, _spine
...
async def _get_user_from_request(request: Request) -> Optional[MCPUser]:
    """Extract and validate user from request headers via the spine."""
    if not _spine:
        logger.warning("Spine not initialized - rejecting request")
        return None
    ...
    if auth_header.startswith("Bearer "):
        ...
        user = await validate_via_spine(_spine, token, mode="core")
        if user:
    ...
    if api_key:
        ...
        user = await validate_via_spine(_spine, api_key, mode="core")
        if user:
            logger.info(f"User authenticated: {user.user_id}")
        else:
...
async def _handle_initialize(params: Dict, request: Request) -> Dict:
    ...
    from novomcp.mcp.router import SERVER_INSTRUCTIONS
