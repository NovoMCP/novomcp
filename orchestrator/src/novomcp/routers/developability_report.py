...
Audit trail is written to managed backend's `funnel_audit_log` via the
...
from novomcp.mcp.auth import MCPUser
from novomcp.mcp.router import get_mcp_user
from novomcp.mcp import router as mcp_router_module
from novomcp.mcp import tools as mcp_tools_module
...
from novomcp.mcp.tools import _sanitize_for_json
from novomcp.models.developability_report import (
...
# MCP per-conversation slots. managed backend persists this verbatim into
...
async def _emit_audit(
    *,
    ...
    """Write the audit row to managed backend.
    ...
    managed backend's single shared pymssql connection is the
...
async def _run_developability_chain(
    ...
    # managed backend availability. Same pattern as MCP autolog.
