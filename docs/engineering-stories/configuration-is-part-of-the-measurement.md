...
`FROZEN` validated. On the production path — a per-axis flat gate (`flat_abs = 0.5·corpus-SD` per endpoint, the fix for the third trap above) — the effect is **+0.1352 SD** beyond a step-order null that no permutation in 500 reached (**z ≈ 14.6**), on 54,312 real homologous series. The headline number is itself gate-dependent: the same effect reads +0.153 SD under the single absolute gate and +0.108 SD under standardised input. That the validated figure moves with the configuration is the whole thesis of this piece, applied to its own result — so it is quoted with its configuration attached, not as a bare number. The one question the study could not settle is whether `FROZEN` is separable from trivial rail-detection: a probability pinned at 0 or 1 cannot move, so "it did not move" is guaranteed rather than informative. That is a limitation of the data, not a weakness of the label — the corpus does not contain enough of the right series to reach significance on the conditional.

Fixing the gate produced one more configuration lesson, and it inverts an intuition. To choose the gate's one free parameter we swept it on both datasets: the 54,312-series primary (z ≈ 16) and a small, underpowered secondary (z ≈ 2). The primary cleared its null — 0 of 500 permutations — at *every* setting, sensible or not; a null-based criterion stops discriminating exactly when power is high. The underpowered dataset was the only one whose signal *degraded* under a bad gate, so it, not the powered one, is what located the usable range. The weak instrument reads the setting the strong one is blind to. Worth stating plainly, because the natural assumption — trust the high-powered result to tune the method — is backwards here.
