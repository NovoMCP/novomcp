...
- **Doesn't touch the closed side.** PRs that add hooks or dependencies pointing at hosted-only infrastructure will be closed. If your idea requires the hosted spine to work, it belongs somewhere else.
- **Preserves the seam.** The `AuthGate` / `CreditMeter` / `AuditSink` interfaces in `novomcp/mcp/spine.py` are the boundary between what's open and what's closed. Changing them requires a discussion in an issue first.
...
- **Do not add hosted-spine features here.** Metering, auth, billing consoles, admin dashboards, those belong in the hosted spine, which is not this project.
